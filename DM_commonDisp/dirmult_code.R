...
## CODE from dirmult
# added: 
# return mom in dirmult
# changed in est estProfLogLik maxit=1e5
##############################################
...
dirmult <- function(data,init,initscalar,epsilon=10^(-4),trace=TRUE,mode){
  ...
  data <- data[rowSums(data)!=0,colSums(data)!=0] # we wan to keep the rows with 0s
  ...
  list(loglik=lik1,ite=ite-1,gamma=as.numeric(gamma),pi=pi,theta=theta, g.mom=g.mom , g0.mom=g0.mom , pi.mom=pi.mom, t.mom=t.mom) ## CHANGE!
}
...
estProfLogLik <- function(data,theta, epsilon=10^(-4), trace=TRUE,initPi,maxit=1e5){
  
  # Lagrange multiplier lambda method!!!
