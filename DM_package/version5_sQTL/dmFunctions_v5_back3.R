...
#   return(NULL)
  ...
  adjCR <- log(det(N * dmInvObsFIMGkm1(pi = piH[-length(piH)], gamma0, y, inv = FALSE)))/2 ## with Gamma functions ## if piH is NULL then:
...
dmFit <- function(dge, group=NULL, dispersion=NULL, mode="constrOptim2", epsilon = 1e-05, maxIte = 1000, verbose=FALSE, mcCores = 20){
  ...
  fit <- mclapply(seq(length(y)), function(g){  
    ...
    f <- dmOneGeneManyGroups(y[[g]], ngroups = ngroups, lgroups = lgroups, igroups = igroups, gamma0 = gamma0, mode = mode, epsilon = epsilon, maxIte = maxIte, verbose = verbose)  
