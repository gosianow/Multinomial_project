...
# ############# test the DM v4 performance 
# name1 <- "fc_g0_s4_keep0s_subsetInf_DM4"
# dge <- dgeOrg
...
# 
# # group=NULL; adjust = FALSE; mode = "constrOptim2"; epsilon = 1e-05; maxIte = 1000; interval = c(0, 1e+5); tol = 1e-02; mcCores=2; verbose=FALSE
# 
# ## run DM pipeline
# timeDMinf <- system.time( dgeDMinf <- dmEstimateCommonDisp(dge, group=NULL, adjust = FALSE, subset = Inf, mode = "constrOptim2", epsilon = 1e-05, maxIte = 1000, interval = c(0, 1e+5), tol = 1e-02, mcCores=40, verbose=FALSE) )
# 
# 
# timeDMhalf <- system.time( dgeDMhalf <- dmEstimateCommonDisp(dge, group=NULL, adjust = FALSE, subset = round(nlevels(dge$genes$gene.id)/2), mode = "constrOptim2", epsilon = 1e-05, maxIte = 1000, interval = c(0, 1e+5), tol = 1e-02, mcCores=40, verbose=FALSE) )
# 
# dgeDM <- dgeDMinf
# 
# write.table(dgeDM$commonDispersion, paste0(out.dir, "/",name1,"_commonDispersion.txt"), quote=F, sep="\t", row.names=F, col.names=F)
# 
# dgeDM <- dmFit(dgeDM, group=NULL, dispersion=NULL, mode="constrOptim2", epsilon = 1e-05, maxIte = 1000, verbose=FALSE, mcCores=40)
# 
# dgeDM <- dmTest(dgeDM, mode="constrOptim2", epsilon = 1e-05, maxIte = 1000, verbose=FALSE, mcCores=40)
# 
...
## run DM pipeline
dgeDM <- dmEstimateCommonDisp(dge, group=NULL, adjust = FALSE, mode = "constrOptim2", epsilon = 1e-05, maxIte = 1000, interval = c(0, 1e+5), tol = 1e-02, mcCores=40, verbose=FALSE)
...
## run DM pipeline
dgeDM <- dmEstimateCommonDisp(dge, group=NULL, adjust = TRUE, mode = "constrOptim2", epsilon = 1e-05, maxIte = 1000, interval = c(0, 1e+5), tol = 1e-02, mcCores=40, verbose=FALSE) 
...
################################################################
name1 <- "htseq_g0_s3_keep0s_subsetInf_DM4"
dge <- dgeOrg
keep <- rowSums(cpm(dge) > 0) >= 3
dge <- dge[keep,]
dge$genes$gene.id <- as.factor(as.character(dge$genes$gene.id))
...
dgeDM <- dgeDM <- dmFit(dgeDM, group=NULL, dispersion=NULL, mode="constrOptim2", epsilon = 1e-05, maxIte = 1000, verbose=FALSE, mcCores=40)
...
################################################################
name1 <- "htseq_g0_s4_keep0s_subsetInf_DM4"
dge <- dgeOrg
keep <- rowSums(cpm(dge) > 0) >= 4
dge <- dge[keep,]
dge$genes$gene.id <- as.factor(as.character(dge$genes$gene.id))
