...
data_org <- dmDSdata(counts = counts, gene_id = gene_id, feature_id = feature_id, sample_id = metadata$sample_id, group = metadata$condition)
...
data <- dmDSdata(counts = counts, gene_id = gene_id, feature_id = feature_id, sample_id = metadata$sample_id, group = metadata$condition)
...
common_dispersion <- dispersion <- dmDS_estimateCommonDispersion(data, disp_adjust = TRUE, disp_interval = c(0, 1e+5), disp_tol = 1e-01, prop_mode = "constrOptimG", prop_tol = 1e-12, verbose=FALSE, BPPARAM = MulticoreParam(workers = 10))
...
table <- dmDS_test(fit, verbose = FALSE, BPPARAM = BPPARAM)
...
dmDS_plotFit(data, gene_id = table[1:1, "gene_id"], fit = fit, table = table, plot_type = c("barplot", "boxplot1", "boxplot2", "lineplot", "ribbonplot")[5], order = FALSE,  plot_full = TRUE, plot_null = FALSE, out_dir = out_name)
