#######################################################
# 
# Created 24 July 2014 / Last updated 31 July 2014

# + updated ROCx functions (with cumsum) super fast
# Different filtering
...
# 
# # ramp <- colorRampPalette(brewer.pal(12,"Set1"))(12)
...
apvs <- table[,c("adjPValue_fc_DM")]
status <- table$status
ROCx_notNorm(pvs, apvs, status, col=colors["fc_DM"], pch=4, ylim=c(0,1), xlim=c(0,1), lwd=4, cex=3, cex.lab=1.5, cex.axis=1.5)
...
pvs <- table[,c("PValue_htseq_dexseq_simes")]
apvs <- table[,c("adjPValue_htseq_dexseq_simes")]
status <- table$status
ROCx_notNorm(pvs, apvs, status,  add=TRUE, col=colors["htseq_dexseq_simes"], pch=4, xlim=c(0,1), lwd=4, cex=3)

legend("bottomright", c("fc_DM", "htseq_DM", "fc_voomex", "htseq_dexseq_simes"), col=colors[c("fc_DM", "htseq_DM", "fc_voomex", "htseq_dexseq_simes")], lty=1, lwd=4, cex=1.5)
dev.off()
...
apvs <- table[,paste0("adjPValue_", methods[1])]
status <- table$status
ROCx_notNorm(pvs, apvs, status, col=colors[methods[1]], pch=4, ylim=c(0,0.9), xlim=c(0,0.8), lwd=4, cex=3, cex.lab=1.5, cex.axis=1.5)
...
#######################################################
# generate TPR vs acieved FDR plots
#######################################################
...
apvs <- table[,paste0("adjPValue_", methods[1])]
status <- table$status
TPRvsFDR(status, apvs, col=colors[methods[1]], xlim=c(0,0.3), ylim=c(0,1), lwd=4, cex=2.5, cex.lab=1.5, cex.axis=1.5)
...
apvs <- table[,paste0("adjPValue_", methods[1])]
status <- table$status
TPRvsFDR(status, apvs, col=colors[methods[1]], xlim=c(0,0.15), ylim=c(0,0.8), lwd=4, cex=2.5, cex.lab=1.5, cex.axis=1.5)
...
for( i in methods[-1]){
  apvs <- table[,paste0("adjPValue_", i)]
  status <- table$status
  TPRvsFDRextend(status, apvs, col=colors[i], add=TRUE, lwd=4, cex=1)
}
...
plot.venn(venne.genes, colors=c(colors[c("fc_DM", "htseq_DM", "fc_voomex", "htseq_dexseq_simes")], True="grey"), venn.methods = c("fc_DM", "htseq_DM", "fc_voomex", "htseq_dexseq_simes", "True"), name=paste0(name, "All_"), margin=0.1, cat.cex=1.4, cex=1.7)
...
counts <- table(table.org[,c("num.ex", "status")])

pdf(paste0(out.dir, "/",name,"barplot_STRAT.pdf"), h=7, w=14)
barplot(t(counts)+1, main="Genes distribution by nr. of exons",xlab="Number of Exons", col=c("darkblue","red"), legend = colnames(counts), xlim=c(0, 30))
dev.off()


table(table.org[table.org$status==1,c("num.ex", "num.diff.ex")])


strat <- c(2, 4, 6, 10, 16, 116)
...
for(j in 1:(length(strat)-1) ){
  ...
  apvs <- table[,paste0("adjPValue_", methods[1])]
  status <- table$status
  TPRvsFDR(status, apvs, col=colors[methods[1]], xlim=c(0,0.3), ylim=c(0,1), lwd=4, cex=2.5, cex.lab=1.5, cex.axis=1.5, main=paste0("Number of exons: ", paste0(strat[j]:( strat[j+1] -1 ), collapse = ", ")) )  
  for( i in methods[-1]){
    apvs <- table[,paste0("adjPValue_", i)]
    status <- table$status
    TPRvsFDR(status, apvs, col=colors[i], add=TRUE, lwd=4, cex=2.5)
