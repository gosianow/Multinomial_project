...
# run on simulated data: Simulations 14 from Katarina
# MISO & DEXSeq counts
...
setwd("/home/gosia/Multinomial_project/Simulations14")
...
# HTSeq/DEXSeq counts:
ecs <- DEXSeqDataSetFromHTSeq(countfiles = paste0("DEXSeq_counts/sim14", metadata$SampleName, "rno_snfb.txt"), sampleData= data.frame(row.names = metadata$SampleName, condition = metadata$condition), design = ~sample + exon + condition:exon)
...
pdf(paste0(out.dir, "/",name,"MDS.pdf"))
plotMDS(dge)
dev.off()


pdf(paste0(out.dir, "/", name ,"voom.pdf"))
...
write.table(top.exon, paste0(out.dir, "/" , name ,"voom_sim14_exon.xls"), sep="\t", row.names=FALSE, quote=F) 
write.table(top.gene, paste0(out.dir, "/" , name , "voom_sim14_gene.xls"), sep="\t", row.names=FALSE, quote=F) 
...
write.table(edgeR_res, paste0(out.dir,  "/" , name , "edgeR_sim14.xls"), sep="\t", row.names=FALSE, quote=F) 
...
table.tmp[!complete.cases(table.tmp), ]
...
### add Results of voomex & edgeR ESTimated here
...
## DEXSeq
out_voom.org <- read.table("Results_voomex/DEXSeq_G2_EST_voom_sim14_gene.xls", header=T)
out_edgeR.org <- read.table(paste0("Results_edgeR/DEXSeq_G2_EST_edgeR_sim14.xls"), header=T)
...
## DEXSeq
out_dexseq.org <- read.table("Results/DEXSeq_sim14_genelevel.xls", header=T)
...
hist(table.all$PValue_voom, col="blue", breaks=50, main="voomex", cex.main=3, cex.lab=1.5, cex.axis = 1.5)
dev.off()
