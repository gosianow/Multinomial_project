...
# use constrOptim2 and constrOptim2G for calculating the proportions
...
out.dir <- paste0("DM_0_1_3/",count.method,"/")
dir.create(out.dir, showWarnings=F, recursive=T)
...
out.dir <- paste0("DM_0_1_3/",count.method,"/",filter.method,"/")
dir.create(out.dir, showWarnings=F, recursive=T)
...
hist(tt, breaks = 100, col = "darkseagreen2", main = paste0(length(tt), " genes \n ", sum(tt) , " transcripts "), xlab = "number of transcripts per gene")
dev.off()
...
filter.methodList <- c("Filtering_DEXSeq", "Filtering0") 
...
## dmOneGeneGroup use tol = .Machine$double.eps
## dmEstimateTagwiseDisp use tol = sqrt(.Machine$double.eps)

for(i in 1:length(modeDispList)){
  # i = 1
  ...
  dgeDM <- dmEstimateTagwiseDisp(dgeDM, group = NULL, adjust = TRUE, mode = mode, epsilon = 1e-05, maxIte = 1000, modeDisp = modeDisp, interval = c(0, 1e+10), tol = sqrt(.Machine$double.eps) , initDisp = 10, initWeirMoM = TRUE, gridLength = 15, gridRange = c(-6, 6), trend = trend, priorDf = 4, span = 0.3, mcCores = mcCores, verbose = FALSE)
